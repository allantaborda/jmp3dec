...
/** An BufferedInputStream that parses Shoutcast's "icy" metadata from the stream.  Gets headers at the beginning and if the
	"icy-metaint" tag is found, it parses and strips in-stream metadata.
	<p>
	<b>The deal with metaint</b>: Icy streams don't try to put tags between MP3 frames the way that ID3 does. Instead, it
	requires the client to strip metadata from the stream before it hits the decoder. You get an <code>icy-metaint</code>
	name/val in the beginning of the stream iff you sent "Icy-Metadata" with value "1" in the request headers
	(SimpleMP3DataSource does this if the "parseStreamMetadata" boolean is true). If this is the case then the value of
	icy-metaint is the amount of real data between metadata blocks. Each block begins with an int indicating how much
	metadata there is -- the block is this value times 16 (it can be, and often is, 0).
	<p>
	Originally thought that "icy" implied Icecast, but this is completely wrong -- real Icecast servers, found through
	www.icecast.net and typified by URLs with a trailing directory (like CalArts School of Music - http://65.165.174.100:8000/som)
	do not have the "ICY 200 OK" magic string or any of the CRLF-separated headers. Apparently, "icy" means "Shoutcast".
	Yep, that's weird.
...
public class IcyInputStream extends BufferedInputStream{
	...
	/* looks like icy streams start start with
	   ...
	   we only get icy-metaint if the http request that created this stream sent the header "icy-metadata:1"
	   in in-line metadata, we read a byte that tells us how  many 16-byte blocks there are (presumably, we
	   still use \r\n for the separator... the block is padded out with 0x00's that we can ignore)
	   // when server is full/down/etc, we get the following for one of the notice lines:
	   icy-notice2:This server has reached its user limit<BR> or
	 ...
	 * If -1, this stream does not have metadata after the header.
	 */
	...
	/**
	 * Reads the initial headers of the stream and adds tags appropriatly. Gets set up to find, read,
	 * and strip blocks of in-line metadata if the <code>icy-metaint</code> header is found.
	 */
	public IcyInputStream(InputStream in) throws IOException{
		super(in);
		tags = new HashMap<>();
		tagParseSupport = new MP3TagParseSupport();
		// read the initial tags here, including the metaint and set the counter for how far we read until the next metadata block (if any).
		readInitialHeaders();
	...
	protected void readInitialHeaders() throws IOException{
		...
		while(!((line = readCRLFLine()).isEmpty())){
			...
			addTag(new IcyTag(line.substring(0, colonIndex), line.substring(colonIndex + 1)));
		}
	}
	...
	protected String readCRLFLine() throws IOException{
		...
		// get the string from the byte[].  i is 1 too high because of read-ahead in crlf block
		return new String(crlfBuffer, 0, i - 1);
	}

	/**
	 * Reads and returns a single byte. If the next byte is a metadata block, then that block is read,
	 * stripped, and parsed before reading and returning the first byte after the metadata block.
	 */
	...
	/**
	 * Reads a block of bytes.  If the next byte is known to be a block of metadata, then that is read, parsed, and stripped, and then a block of
	 * bytes is read and returned. Otherwise, it may read up to but not into the next metadata block if <code>bytesUntilNextMetadata &lt; length</code>.
	 */
	public int read(byte[] buf, int offset, int length) throws IOException{
		// If not on metadata, do the usual read so long as we don't read past metadata
		if(bytesUntilNextMetadata > 0){
			...
			readMetadata();
			// now as above, except that we reset bytesUntilNextMetadata differently 
	...
	/**
	 * Read the next segment of metadata.  The stream <b>must</b> be right on the segment, ie, the next byte to read
	 * is the metadata block count.  The metadata is parsed and new tags are added with addTag(), which fires events.
	 */
	...
	protected void parseInlineIcyTags(byte[] tagBlock){
		for(String tagString : new String(tagBlock, ISO_8859_1).split(INLINE_TAG_SEPARATORS)){
	...
	public MP3Tag[] getTags(){
		return tags.values().toArray(new MP3Tag[tags.values().size()]);
	}

	/** Returns a HashMap of all headers and in-stream tags parsed so far. */
	public HashMap<String, Object> getTagHash(){
		return tags;
	}
	...
	public static void main(String args[]){
		...
		try{
			...
			try(IcyInputStream icy = new IcyInputStream(new BufferedInputStream(conn.getInputStream()))){
				while(icy.available() > -1) icy.read(chow, 0, chow.length);
			}
		}catch(Exception e){
			e.printStackTrace();
		}
