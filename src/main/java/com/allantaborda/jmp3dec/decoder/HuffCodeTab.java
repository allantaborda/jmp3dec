...
final class HuffCodeTab{
	...
	public static int huffman_decoder(HuffCodeTab h, int[] x, int[] y, int[] v, int[] w, BitReserve br){
		// array of all huffcodtable headers 0..31 Huffman code table 0..31 32,33 count1-tables
		int dmask = 1 << ((4 * 8) - 1);
		// int hs = 4 * 8;
		int level;
		int point = 0;
		...
		if(h.tablename0 == '3' && (h.tablename1 == '2' || h.tablename1 == '3')) {
			...
			// Process sign and escape encodings for dual tables. x and y are reversed in the test bitstream. Reverse x and y here to make test bitstream work.
			if(h.linbits != 0)
				if((h.xlen - 1) == x[0]) x[0] += br.hgetbits(h.linbits);
			if(x[0] != 0)
				if(br.hget1bit() != 0) x[0] = -x[0];
			if(h.linbits != 0)
				if((h.ylen - 1) == y[0]) y[0] += br.hgetbits(h.linbits);
			if(y[0] != 0)
				if(br.hget1bit() != 0) y[0] = -y[0];
		}
		return error;
