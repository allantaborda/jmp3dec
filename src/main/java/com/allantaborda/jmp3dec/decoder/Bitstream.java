 ...
 * <b>REVIEW:</b> much of the parsing currently occurs in the various decoders. This should be moved into this class and associated inner classes.
 */
public final class Bitstream implements BitstreamErrors{
	...
	public Bitstream(InputStream in){
		in = new BufferedInputStream(Objects.requireNonNull(in));
		loadID3v2(in);
		firstframe = true;
	 ...
	 * @throws IOException If an I/O error occourrs.
	 * @author JavaZOOM
	 */
	...
	/**
	 * Close the Bitstream.
	 * @throws BitstreamException If a stream error occourrs.
	 */
	...
	public boolean isSyncCurrentPosition(int syncmode) throws BitstreamException{
		...
		boolean sync = false;
		if(read == 0) sync = true;
		else if(read == 4) sync = isSyncMark(headerstring, syncmode, syncword);
		return sync;
	}

	// REVIEW: this class should provide inner classes to parse the frame contents. Eventually, readBits will be removed.
	...
	public boolean isSyncMark(int headerstring, int syncmode, int word){
		boolean sync = false;
		if(syncmode == INITIAL_SYNC) sync = ((headerstring & 0xFFE00000) == 0xFFE00000); // SZD: MPEG 2.5
		else sync = ((headerstring & 0xFFF80C00) == word) && (((headerstring & 0x000000C0) == 0x000000C0) == single_ch_mode);
	...
	public int get_bits(int number_of_bits){
		...
		// E.B : There is a problem here, wordpointer could be -1 ?!
	 ...
	 * @exception BitstreamException If the specified number of bytes could not be read from the stream.
	 */
