...
final class LayerIIIDecoder implements FrameDecoder{
	...
	private static final float two_to_negative_half_pow[] = { 1.0000000000E+00f, 7.0710678119E-01f, 5.0000000000E-01f,
	...
	private static final float TAN12[] = { 0.0f, 0.26794919f, 0.57735027f, 1.0f, 1.73205081f, 3.73205081f, 9.9999999e10f,
	...
	private static final float win[][] = { { -1.6141214951E-02f, -5.3603178919E-02f, -1.0070713296E-01f,
			-1.6280817573E-01f, -4.9999999679E-01f, -3.8388735032E-01f, -6.2061144372E-01f, -1.1659756083E+00f,
			-3.8720752656E+00f, -4.2256286556E+00f, -1.5195289984E+00f, -9.7416483388E-01f, -7.3744074053E-01f,
			-1.2071067773E+00f, -5.1636156596E-01f, -4.5426052317E-01f, -4.0715656898E-01f, -3.6969460527E-01f,
			-3.3876269197E-01f, -3.1242222492E-01f, -2.8939587111E-01f, -2.6880081906E-01f, -5.0000000266E-01f,
			-2.3251417468E-01f, -2.1596714708E-01f, -2.0004979098E-01f, -1.8449493497E-01f, -1.6905846094E-01f,
			-1.5350360518E-01f, -1.3758624925E-01f, -1.2103922149E-01f, -2.0710679058E-01f, -8.4752577594E-02f,
			...
			{ 0.0000000000E+00f, 0.0000000000E+00f, 0.0000000000E+00f, 0.0000000000E+00f, 0.0000000000E+00f,
					...
					-1.4790705759E-02f } };
	private static final int nr_of_sfb_block[][][] = { { { 6, 5, 5, 5 }, { 9, 9, 9, 9 }, { 6, 9, 9, 9 } }, { { 6, 5, 7, 3 }, { 9, 9, 12, 6 }, { 6, 9, 12, 6 } },
	...
	// MDM: tsOutCopy and rawout do not need initializing, so the arrays can be reused.
	private float[] tsOutCopy = new float[18];
	private float[] rawout = new float[36];
	private SBI[] sfBandIndex; // Init in the constructor.

	static{
		t_43 = new float[8192];
		for(int i = 0; i < 8192; i++) t_43[i] = (float) Math.pow(i, d43);
	}

	// REVIEW: these constructor arguments should be moved to the decodeFrame() method, where possible, so that one
	public LayerIIIDecoder(Bitstream stream0, Header header0, SynthesisFilter filtera, SynthesisFilter filterb, Obuffer buffer0, int which_ch0){
		...
		for(int ch = 0; ch < 2; ch++){
			for(int j = 0; j < 576; j++) prevblck[ch][j] = 0.0f;
		}
	...
	public void decodeFrame(){
		...
		int flush_main, gr, ch, ss, sb, sb18, main_data_end, bytes_to_discard, i;
		get_side_info();
		for(i = 0; i < nSlots; i++) br.hputbuf(stream.get_bits(8));
		...
		for(gr = 0; gr < max_gr; gr++){
			for(ch = 0; ch < channels; ch++){
				...
				if(header.version() == Header.MPEG1) get_scale_factors(ch, gr);
				// MPEG-2 LSF, SZD: MPEG-2.5 LSF
				else get_LSF_scale_factors(ch, gr);
			...
			for(ch = first_channel; ch <= last_channel; ch++){
				...
				for(sb18 = 18; sb18 < 576; sb18 += 36){ // Frequency inversion
					for(ss = 1; ss < SSLIMIT; ss += 2) out_1d[sb18 + ss] = -out_1d[sb18 + ss];
				}
	...
	private boolean get_side_info(){
		...
		if(header.version() == Header.MPEG1){
			...
			for(ch = 0; ch < channels; ch++){
				for(int c = 0; c < 4; c++) si.ch[ch].scfsi[c] = stream.get_bits(1);
			}
			for(gr = 0; gr < 2; gr++){
				for(ch = 0; ch < channels; ch++){
					...
					if(si.ch[ch].gr[gr].window_switching_flag != 0){
						...
						for(int c = 0; c < 3; c++) si.ch[ch].gr[gr].subblock_gain[c] = stream.get_bits(3);
						// Set region_count parameters since they are implicit in this case.
						// Side info bad: block_type == 0 in split block
						if(si.ch[ch].gr[gr].block_type == 0) return false;
						else if (si.ch[ch].gr[gr].block_type == 2 && si.ch[ch].gr[gr].mixed_block_flag == 0) si.ch[ch].gr[gr].region0_count = 8;
						else si.ch[ch].gr[gr].region0_count = 7;
						si.ch[ch].gr[gr].region1_count = 20 - si.ch[ch].gr[gr].region0_count;
					}else{
						for(int c = 0; c < 3; c++) si.ch[ch].gr[gr].table_select[c] = stream.get_bits(5);
			...
			si.private_bits = stream.get_bits(channels == 1 ? 1 : 2);
			for(ch = 0; ch < channels; ch++){
				...
				if(si.ch[ch].gr[0].window_switching_flag != 0){
					...
					for(int c = 0; c < 3; c++) si.ch[ch].gr[0].subblock_gain[c] = stream.get_bits(3);
					// Set region_count parameters since they are implicit in this case.
					// Side info bad: block_type == 0 in split block
					if(si.ch[ch].gr[0].block_type == 0) return false;
					else if (si.ch[ch].gr[0].block_type == 2 && si.ch[ch].gr[0].mixed_block_flag == 0) si.ch[ch].gr[0].region0_count = 8;
					else{
	...
	private void get_scale_factors(int ch, int gr){
		...
		if(gr_info.window_switching_flag != 0 && gr_info.block_type == 2){
			if(gr_info.mixed_block_flag != 0){ // MIXED
				for(sfb = 0; sfb < 8; sfb++) scalefac[ch].l[sfb] = br.hgetbits(slen[0][gr_info.scalefac_compress]);
				for(sfb = 3; sfb < 6; sfb++){ 
					for(window = 0; window < 3; window++) scalefac[ch].s[window][sfb] = br.hgetbits(slen[0][gr_info.scalefac_compress]);
				}
				for(sfb = 6; sfb < 12; sfb++){
					for(window = 0; window < 3; window++) scalefac[ch].s[window][sfb] = br.hgetbits(slen[1][gr_info.scalefac_compress]);
				}
				for(sfb = 12, window = 0; window < 3; window++) scalefac[ch].s[window][sfb] = 0;
			}else{ // SHORT
				for(int c = 0; c < 3; c++){
					for(int d = 0; d < 6; d++) scalefac[ch].s[c][d] = br.hgetbits(length0);
				}
				for(int c = 0; c < 3; c++){
					for(int d = 6; d < 12; d++) scalefac[ch].s[c][d] = br.hgetbits(length1);
					scalefac[ch].s[c][12] = 0;
				}
			} // SHORT
		}else{ // LONG types 0,1,3
			if(si.ch[ch].scfsi[0] == 0 || gr == 0){
				for(int c = 0; c < 6; c++) scalefac[ch].l[c] = br.hgetbits(length0);
			}
			if(si.ch[ch].scfsi[1] == 0 || gr == 0){
				for(int c = 6; c < 11; c++) scalefac[ch].l[c] = br.hgetbits(length0);
			}
			if(si.ch[ch].scfsi[2] == 0 || gr == 0){
				for(int c = 11; c < 16; c++) scalefac[ch].l[c] = br.hgetbits(length1);
			}
			if(si.ch[ch].scfsi[3] == 0 || gr == 0){
				for(int c = 16; c < 21; c++) scalefac[ch].l[c] = br.hgetbits(length1);
			}
			scalefac[ch].l[21] = 0;
			scalefac[ch].l[22] = 0;
		}
	...
	private void get_LSF_scale_data(int ch, int gr){
		...
		// why 45, not 54?
		for(int x = 0; x < 45; x++) scalefac_buffer[x] = 0;
		for(int i = 0, m = 0; i < 4; i++){
			for(int j = 0; j < nr_of_sfb_block[blocknumber][blocktypenumber][i]; j++) scalefac_buffer[m++] = (new_slen[i] == 0) ? 0 : br.hgetbits(new_slen[i]);
		}
	}
	...
	private void get_LSF_scale_factors(int ch, int gr){
		...
		if(gr_info.window_switching_flag != 0 && gr_info.block_type == 2){
			if(gr_info.mixed_block_flag != 0){ // MIXED
				for(sfb = 0; sfb < 8; sfb++) scalefac[ch].l[sfb] = scalefac_buffer[m++];
				for(sfb = 3; sfb < 12; sfb++){
					for(window = 0; window < 3; window++) scalefac[ch].s[window][sfb] = scalefac_buffer[m++];
				}
				for(window = 0; window < 3; window++) scalefac[ch].s[window][12] = 0;
			}else{ // SHORT
			...
			for(sfb = 0; sfb < 21; sfb++) scalefac[ch].l[sfb] = scalefac_buffer[m++];
	...
	private void huffman_decode(int ch, int gr){
		...
		int num_bits, region1Start, region2Start, index, buf, buf1;
		HuffCodeTab h;
		...
		// Zero out rest
		if(index < 576) nonzero[ch] = index;
		else nonzero[ch] = 576;
		if(index < 0) index = 0;
		// may not be necessary
		for(; index < 576; index++) is_1d[index] = 0;
	}
	...
	private void dequantize_sample(float xr[][], int ch, int gr){
		GrInfoS gr_info = (si.ch[ch].gr[gr]);
		int cb = 0, next_cb_boundary, cb_begin = 0, cb_width = 0, index = 0, t_index;
		float g_gain;
		float[][] xr_1d = xr;
		// choose correct scalefactor band per block type, initalize boundary
		if(gr_info.window_switching_flag != 0 && gr_info.block_type == 2){
			if(gr_info.mixed_block_flag != 0) next_cb_boundary = sfBandIndex[sfreq].l[1]; // LONG blocks: 0,1,3
			else{
		...
		for(int j = 0; j < nonzero[ch]; j++){
			// Modif E.B 02/22/99
			int reste = j % SSLIMIT, quotien = (j - reste) / SSLIMIT;
	...
	private void reorder(float xr[][], int ch, int gr){
		GrInfoS gr_info = (si.ch[ch].gr[gr]);
		int freq, freq3, sfb, sfb_start, sfb_lines, src_line, des_line;
		float[][] xr_1d = xr;
		if(gr_info.window_switching_flag != 0 && gr_info.block_type == 2){
			for(int c = 0; c < 576; c++) out_1d[c] = 0.0f;
			if(gr_info.mixed_block_flag != 0){
				// NO REORDER FOR LOW 2 SUBBANDS
				for(int c = 0; c < 36; c++){
					// Modif E.B 02/22/99
					int reste = c % SSLIMIT, quotien = (c - reste) / SSLIMIT;
					out_1d[c] = xr_1d[quotien][reste];
				}
				...
				for(int c = 0; c < 576; c++){
					int j = reorder_table[sfreq][c], reste = j % SSLIMIT, quotien = (j - reste) / SSLIMIT;
					out_1d[c] = xr_1d[quotien][reste];
				}
			}
		}else{ // long blocks
	...
	private void stereo(int gr){
		...
		if(channels == 1){ // mono , bypass xr[0][][] to lr[0][][]
			...
			int mode_ext = header.mode_extension(), sfb, i, lines, temp, temp2;
	...
	private void antialias(int ch, int gr){
		...
		if(gr_info.window_switching_flag != 0 && gr_info.mixed_block_flag != 0 && gr_info.block_type == 2) sb18lim = 18;
		else sb18lim = 558;
		for(sb18 = 0; sb18 < sb18lim; sb18 += 18){
			for(ss = 0; ss < 8; ss++){
				int src_idx1 = sb18 + 17 - ss, src_idx2 = sb18 + 18 + ss;
				float bu = out_1d[src_idx1], bd = out_1d[src_idx2];
	...
	private void hybrid(int ch, int gr){
		...
		for(sb18 = 0; sb18 < 576; sb18 += 18){
			...
			tsOut = out_1d;
			// Modif E.B 02/22/99
			for(int cc = 0; cc < 18; cc++) tsOutCopy[cc] = tsOut[cc + sb18];
			...
			prvblk = prevblck;
			for(int c = 0; c < 18; c++){
				tsOut[c + sb18] = rawout[c] + prvblk[ch][c + sb18];
				prvblk[ch][c + sb18] = rawout[c + 18];
			}
		}
	}
	...
	public void inv_mdct(float[] in, float[] out, int block_type){
		float[] win_bt;
		float tmpf_0, tmpf_1, tmpf_2, tmpf_3, tmpf_4, tmpf_5, tmpf_6, tmpf_7, tmpf_8, tmpf_9, tmpf_10, tmpf_11, tmpf_12, tmpf_13, tmpf_14, tmpf_15, tmpf_16, tmpf_17;
		...
		if(block_type == 2){
			for(int c = 0; c < 36; c++) out[c] = 0.0f;
			for(int i = 0, six_i = 0; i < 3; i++){
				// 12 point IMDCT
				// Begin 12 point IDCT
			...
			for(int c = 17; c > 0; c--) in[c] += in[c - 1];
			// 18 point IDCT for odd indices
			// input aliasing for 18 point IDCT
			for(int c = 17; c > 2; c -= 2) in[c] += in[c - 2];
			float tmp0, tmp1, tmp2, tmp3, tmp4, tmp0_, tmp1_, tmp2_, tmp3_, tmp0o, tmp1o, tmp2o, tmp3o, tmp4o, tmp0_o, tmp1_o, tmp2_o, tmp3_o;
			...
			float i00 = in[0] + in[0], iip12 = i00 + in[12];
			...
			// 5 points on odd indices (not realy an IDCT)
			float i0 = in[0 + 1] + in[0 + 1], i0p12 = i0 + in[12 + 1];
	...
	private static int[] reorder(int scalefac_band[]){ // SZD: converted from LAME
		...
		for(int sfb = 0; sfb < 13; sfb++){
			...
			for(int window = 0; window < 3; window++){
				for(int i = start; i < end; i++) ix[3 * i + window] = j++;
			}
		}
		return ix;
	}
	...
	static class IIISideInfoT {
		int main_data_begin, private_bits;
		Temporaire[] ch;
