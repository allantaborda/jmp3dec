...
class LayerIDecoder implements FrameDecoder{
	...
	public LayerIDecoder(Bitstream stream0, Header header0, SynthesisFilter filtera, SynthesisFilter filterb, Obuffer buffer0, int which_ch0){
		crc = new Crc16();
		stream = stream0;
		header = header0;
		filter1 = filtera;
	...
	protected void createSubbands(){
		...
		if(mode == Header.SINGLE_CHANNEL){
			for(i = 0; i < num_subbands; ++i) subbands[i] = new SubbandLayer1(i);
	...
	static interface Subband {
		/* Changes from version 1.1 to 1.2: - array size increased by one, although a scalefactor with index 63 is illegal (to prevent segmentation faults). */
		...
		void read_allocation(Bitstream stream, Header header, Crc16 crc) throws DecoderException;
	...
	static class SubbandLayer1 implements Subband{
		...
		public SubbandLayer1(int subbands){
			subbandnumber = subbands;
			samplenumber = 0;
		}
		...
		public boolean put_next_sample(int channels, SynthesisFilter filter1, SynthesisFilter filter2){
			if(allocation != 0 && channels != OutputChannels.RIGHT_CHANNEL) filter1.input_sample((sample * factor + offset) * scalefactor, subbandnumber);
			return true;
		}
	};
	...
	static class SubbandLayer1IntensityStereo extends SubbandLayer1{
		...
		public boolean put_next_sample(int channels, SynthesisFilter filter1, SynthesisFilter filter2){
			if(allocation != 0){
				...
				if(channels == OutputChannels.BOTH_CHANNELS || channels == OutputChannels.LEFT_CHANNEL) filter1.input_sample(sample * scalefactor, subbandnumber);
				if(channels == OutputChannels.BOTH_CHANNELS || channels == OutputChannels.RIGHT_CHANNEL) filter1.input_sample(sample * channel2_scalefactor, subbandnumber);
			}
			return true;
		}
	};
	...
	static class SubbandLayer1Stereo extends SubbandLayer1{
		...
		public boolean read_sampledata(Bitstream stream){
			boolean ret = super.read_sampledata(stream);
