...
public final class Equalizer{
	...
	public void setFrom(float[] eq){
		...
		for(int i = 0; i < max; i++) settings[i] = limit(eq[i]);
	}

	public void setFrom(EQFunction eq){
		reset();
		int max = BANDS;
		for(int i = 0; i < max; i++) settings[i] = limit(eq.getBand(i));
	}
	...
	public void setFrom(Equalizer eq){
		if(eq != this) setFrom(eq.settings);
	}
	...
	/**
	 * Retrieves an array of floats whose values represent a scaling factor that can be applied
	 * to linear samples in each band to provide the equalization represented by this instance.
	...
	/**
	 * Converts an equalizer band setting to a sample factor. The factor is  determined by the
	 * function f = 2^n where n is the equalizer band setting in the range [-1.0, 1.0].
	 */
	float getBandFactor(float eq){
		if(eq == BAND_NOT_PRESENT) return 0.0f;
		return (float) Math.pow(2.0, eq);
	}

	@FunctionalInterface
	public interface EQFunction{
		/**
