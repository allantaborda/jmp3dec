...
public final class Header{
	...
	public static final String bitrate_str[][][] ={{
			...
			{{ "free format", "32 kbit/s", "48 kbit/s", "56 kbit/s", "64 kbit/s", "80 kbit/s", "96 kbit/s",
					...
					{ "free format", "8 kbit/s", "16 kbit/s", "24 kbit/s", "32 kbit/s", "40 kbit/s", "48 kbit/s",
							"56 kbit/s", "64 kbit/s", "80 kbit/s", "96 kbit/s", "112 kbit/s", "128 kbit/s",
							"144 kbit/s", "160 kbit/s", "forbidden" } }, };
	public static final int[][] frequencies = {{ 22050, 24000, 16000, 1 },{ 44100, 48000, 32000, 1 }, { 11025, 12000, 8000, 1 } }; // SZD: MPEG25
	...
	public String toString(){
		...
		buffer.append(" frame ").append(mode_string());
		buffer.append(' ').append(version_string());
		if(!checksums()) buffer.append(" no");
		buffer.append(" checksums").append(' ');
		buffer.append(sample_frequency_string());
		buffer.append(',').append(' ');
		return buffer.append(bitrate_string()).toString();
	}

	/** Read a 32-bit header from the bitstream. */
	void read_header(Bitstream stream, Crc16[] crcp) throws BitstreamException{
		int headerstring, channel_bitrate;
		boolean sync = false;
		do{
			...
			if(h_layer == 1) h_number_of_subbands = 32;
				...
				}else if (h_sample_frequency == FOURTYEIGHT || (channel_bitrate >= 3 && channel_bitrate <= 5)) h_number_of_subbands = 27;
			...
			if(framesize >= 0 && framesizeloaded != framesize){
				// Data loaded does not match to expected framesize, it might be an ID3v1 TAG. (Fix 11/17/04).
	...
	void parseVBR(byte[] firstframe) throws BitstreamException{
		...
		if(h_version == MPEG1) offset = (h_mode == SINGLE_CHANNEL ? 21 : 36) - 4;
		else offset = (h_mode == SINGLE_CHANNEL ? 13 : 21) - 4;
		try{
			...
			if(h_layer == 3){
				if(h_version == MPEG1){
					nSlots = framesize - (h_mode == SINGLE_CHANNEL ? 17 : 32) // side info size
							- (h_protection_bit == 0 ? 2 : 0) // CRC size
							- 4; // header size
	...
	public String layer_string(){
		if(h_layer == 1) return "I";
		if(h_layer == 2) return "II";
		if(h_layer == 3) return "III";
		return null;
	}

	/**
	 * Return Bitrate as string.
	 * @return bitrate in bps.
	 */
	public String bitrate_string(){
		return h_vbr ? Integer.toString(bitrate() / 1000) + " kb/s" : bitrate_str[h_version][h_layer - 1][h_bitrate_index];
	}
	 ...
	 */
	public int bitrate(){
		return h_vbr ? ((int) ((h_vbr_bytes * 8) / (ms_per_frame() * h_vbr_frames))) * 1000 : bitrates[h_version][h_layer - 1][h_bitrate_index];
	}
	...
	public String sample_frequency_string(){
		if(h_sample_frequency == THIRTYTWO){
			...
			return "8 kHz";
		}else if(h_sample_frequency == FOURTYFOUR_POINT_ONE){
	...
	/**
	 * Returns Mode.
	 * @return "Stereo" or "Joint stereo" or "Dual channel" or "Single channel".
	 */
	public String mode_string(){
		if(h_mode == STEREO) return "Stereo";
		if(h_mode == JOINT_STEREO) return "Joint stereo";
		if(h_mode == DUAL_CHANNEL) return "Dual channel";
		if(h_mode == SINGLE_CHANNEL) return "Single channel";
		return null;
	}
	...
	public String version_string(){
		if(h_version == MPEG1) return "MPEG-1";
		if(h_version == MPEG2_LSF) return "MPEG-2 LSF";
		if(h_version == MPEG25_LSF) return "MPEG-2.5 LSF";
		return null;
	}
	...
	/**
	 * Returns Intensity Stereo. (Layer II joint stereo only). Returns the number of subbands
	 * which are in stereo mode, subbands above that limit are in intensity stereo mode.
	 * @return Intensity.
	 */
